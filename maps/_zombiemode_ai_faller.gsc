...
get_available_fall_locations()
{
	...
	iprintln("# of roof spawners avaiable " + level.zombie_fall_spawners.count);
...
{
	self endon("death");
	self.fall_counter = level.fall_counter;
	i = self.fall_counter;
	level.fall_counter++;
	...
	if( spots.size < 1 )
	{
		...
		self unlink();
		self.anchor delete();
		IPrintLn(i + ": No available fall locations for zombie");
		...
		//add back zombie to total count and clean up
		//This tries to re-enable the spawn, but will always fail as 
		//there was not spot to attack to this zombie to begin with.
		self delayThread( 0.1, ::zombie_faller_delete );

		if(getDvarInt("fix_ceiling") == 1) {
			iprintln("Fixing ceiling spawns");
			setup_faller_locations();
			setDvar("fix_ceiling", 0);
		}
		return;
	}
	...
	//TTS - disable spawn while zombie climbs out
	windowStart = getTime();
	...
	oppTime =(windowEnd - windowStart) / 1000;
	iprintln(i + ": Opportunity window - " + oppTime);
...
zombie_faller_enable_location()
{
	...
	{
		//TTS - check enable
		// iprintln(i + ": Spawn has been set to enabled");
		self.zombie_faller_location.is_enabled = true;
		self.zombie_faller_location = undefined;
	}
	else {
		// iPrintLn(i + ": Zombie faller location was not set!");
	}
}
...
zombie_faller_death_wait()
{
	...
	if(isDefined(self.zombie_faller_location)) {
		// iprintln(i + " on death - fall location is defined");
	}
	else {
		// iprintln(i + " on death - fall not defined");
	}
